  ...
  try {
    ...
    if (!invoice_id && !estimate_id) {
      return res.status(400).json({
        message:
          'Task must be associated with either an Invoice or an Estimate.',
      });
    }
...
exports.updateTask = async (req, res) => {
  try {
    ...
    task.invoice_id = invoice_id ?? task.invoice_id;
    task.estimate_id = estimate_id ?? task.estimate_id;
    task.hourly_rate = hourly_rate ?? task.hourly_rate;
